        ...
        try:
            for username in self.usernames:
                ...
                if not user:
                    ...
                    continue
                elif user and user['is_private'] and user['edge_owner_to_timeline_media']['count'] > 0 and not user['edge_owner_to_timeline_media']['edges']:
...
def nsfw_api_batch_post(input, username):
    """
    ...
    try:
        ...
        with open(os.path.join(basedir, username[0], 'scores_posts.json'), 'w') as outfile:
...
def main():
    ...
    parser.add_argument('--profile-metadata', '--profile_metadata', action='store_true', default=True,
                        help='Save profile metadata to json file')
    parser.add_argument('--proxies', default={}, help='Maximum number of items to scrape')
    parser.add_argument('--include-location', '--include_location', action='store_true', default=True,
    ...
    # Check the urls of the retrieved media
    # Let's obtain all the scores for the scrapped images

    print("Calling NSFW API in Heroku for scores retrieval")

    if scraper.urls.__len__() >= 100:
        preds = nsfw_api_batch_post(scraper.urls[0:200], args.username)
    else:
        preds = nsfw_api_batch_post(scraper.urls, args.username)
